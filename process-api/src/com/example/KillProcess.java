...
public class KillProcess {
	...
	public static void main(String[] args) {
		...
		Predicate<ProcessHandle> processCriteria = p -> {
			Optional<String> cmd = p.info().command();
			if (cmd.isPresent()) {
				Path path = Paths.get(cmd.get());
				System.out.println(path.getFileName());
				return Arrays.stream(args).peek(System.out::println)
						.anyMatch(arg -> path.getFileName().toString().contains(arg));
			}
			return false;
		};
		ProcessHandle.allProcesses().filter(p -> p.isAlive()).filter(processCriteria)
